...
export default function DetallePedidoPage() {
  ...
  // Usamos limit 1000 por defecto y filtramos en el cliente si no hay match server-side,
  // dado que esta tabla puede ser enorme, idealmente buscaríamos por ID de pedido exacto.
  const { rows: detalles, loading, error, insertRow, updateRow, deleteRow } = useSupabaseTable(TABLE, { 
    orderBy: ID_COLUMN, 
    ascending: false
  })

  // Cargar algunos pedidos recientes
  const { rows: pedidos } = useSupabaseTable('pedido', { orderBy: 'id', ascending: false, limit: 100 })
  ...
  const filteredData = useMemo(() => {
    ...
    return detalles.filter(r => 
      String(r.pedido).includes(lower) || 
  ...
  const columns = [
    { key: ID_COLUMN, label: '#' },
    { key: 'pedido', label: 'ID Pedido', render: (row) => `#${row.pedido}` },
    { key: 'producto', label: 'Producto', render: (row) => productoMap[row.producto] ?? row.producto },
    { key: 'cantidad_solicitada', label: 'Cantidad', render: (row) => Number(row.cantidad_solicitada) },
    { key: 'precio_unitario_historico', label: 'P. Unitario', render: (row) => `S/ ${Number(row.precio_unitario_historico).toFixed(2)}` },
    { key: 'subtotal', label: 'Subtotal', render: (row) => `S/ ${(Number(row.cantidad_solicitada) * Number(row.precio_unitario_historico)).toFixed(2)}` },
  ]
  ...
  const fields = [
    ...
    { key: 'cantidad_solicitada', label: 'Cantidad', type: 'number', required: true },
    { key: 'precio_unitario_historico', label: 'Precio Unitario (S/)', type: 'number', required: true },
  ]
  ...
  const handleDelete = async (row) => {
    if (!confirm(`¿Eliminar el detalle del pedido #${row.pedido}?`)) return
    try {
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="font-mono text-xs uppercase tracking-widest text-tomato-400">Ventas · Módulo</p>
          <h1 className="font-display text-3xl text-semolina-100">Detalles de Pedido</h1>
        </div>
